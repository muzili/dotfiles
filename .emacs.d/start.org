...
;  (add-to-list 'eglot-server-programs '((c++-mode c-mode) "clangd-6.0"))
...
(use-package ccls
  ...
  (setq ccls-extra-init-params '(:index (:comments 2) :completion (:detailedLabel t) :index (:reparseForDependency 1) :clang (:resourceDir "/usr/local/clang/7.0.0")))
...
(use-package lsp-java
  ...
  (progn 
    (setq lsp-java-vmargs '("-noverify" "-Xmx4G" "-Xmn524m" "-XX:PermSize=1024m" "-XX:PermSize=502m" "-XX:MaxPermSize=1024m" "-Xss4m" "-XX:ParallelGCThreads=8" "-XX:ReservedCodeCacheSize=540m" "-XX:+UseCompressedOops" "-XX:ConcGCThreads=8"  "-XX:-TraceClassUnloading" "-XX:+AlwaysPreTouch" "-XX:+TieredCompilation" "-XX:SoftRefLRUPolicyMSPerMB=50" "-XX:+OptimizeStringConcat"  "-XX:-CITime" "-XX:-PrintConcurrentLocks" "-server" "-XX:+UseG1GC" )
